...
pub fn real_main() {
    ...
    let _is_3d = std::env::var("3d").map(|x| x == "1").unwrap_or(true);
    let window = WindowBuilder::new()
        .with_title("BankClient")
    ...
    match WindowManager::new(window, &event_loop) {
        Ok(am) => {
            ...
            am.run_loop(event_loop, state::InitState::new(Box::new(MainMenu::default())));
        }
        Err(e) => {
